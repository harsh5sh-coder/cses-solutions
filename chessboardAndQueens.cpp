...
vector<bool> col, mainDiagonal, antiDiagonal;
    
void backtrack(int r, vector<vector<string>>& ans, int n, vector<string>& board) {
    if(r == n) {
        ans.push_back(board);
        return;
    }
    for(int j = 0; j < n; j++) {
        if(col[j] || mainDiagonal[r - j + n - 1] || antiDiagonal[r + j] || board[r][j] == '*') continue; 
        // this is a simple mathematical concept: 
        // 1. All entries on anti diagonal have row + col value; same always.
        // 2. All entries on main diagonal have row - col value; same always. but to avoid negative index we are just adding n - 1 to keep values always non-negative.
        col[j] = true;
        mainDiagonal[r - j + n - 1] = true;
        antiDiagonal[r + j] = true;
        board[r][j] = 'Q';
 
        backtrack(r + 1, ans, n, board);
 
        col[j] = false;
        mainDiagonal[r - j + n - 1] = false;
        antiDiagonal[r + j] = false;
        board[r][j] = '.';
    }
 
}
 
int solveNQueens(int n, vector<string>& board) {
    // basic idea is to avoid queen attacks on each other by checking row, column, main diagonal and anti-diagonal.
    col.assign(n, false);
    mainDiagonal.assign(2*n - 1, false);
    antiDiagonal.assign(2*n - 1, false);
 
    vector<vector<string>> ans;
 
    backtrack(0, ans, n, board);
    return ans.size();
}
...
signed main()
{
    ...
    cout<<solveNQueens(8, board)<<endl;
    return 0;
}

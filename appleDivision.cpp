...
void generateAllPairs(int n, int curIndex, ll curSum, vector<int>& vec) {
    
    
    if(curIndex == n) {
        ll temp = totalSum - curSum;
        ans = min(ans, abs(temp - curSum));
        return;
    }
 
    generateAllPairs(n, curIndex + 1, curSum + vec[curIndex], vec);
    generateAllPairs(n, curIndex + 1, curSum, vec);
}

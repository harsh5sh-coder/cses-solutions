...
signed main()
{
    ...
    string available = "ABCD";
    
//  Core Logic: 
// We only have 4 letters {A,B,C,D}. Process cells in row-major order and at cell (i,j) choose any letter from {A,B,C,D} that:
// is different from the original letter in that cell, and
// is different from the already assigned neighbour above (i-1,j) (if exists), and
// is different from the already assigned neighbour left (i,j-1) (if exists).
// When we assign in row-major order we only need to avoid up and left — the right and down neighbors will be assigned later and will avoid conflicts with this cell.
// At most 3 letters can be forbidden (original, up, left). 

// Most Important: With 4 letters there is always at least one choice → greedy always succeeds.
    
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            bool found = false;
            for(char c: available) {
                if(c == input[i][j]) continue;
                if(i > 0 && c == input[i - 1][j]) continue;
                if(j > 0 && c == input[i][j - 1]) continue;
                input[i][j] = c;
                found = true;
                break;
            }
            if(!found) {
                cout<<"IMPOSSIBLE"<<endl; // in case there are less than 4 letters, then it might break.
                return 0;
            }
        }
    }

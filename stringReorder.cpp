...
signed main()
{
    ...
    for(int i = 0; i < n; i++) {
        for(char ch = 'A'; ch <= 'Z'; ch++) {
            if(freq[ch - 'A'] > 0 && prev != ch) {
                freq[ch - 'A'] -= 1;
                int remaining = n - i - 1;
                int maxRem = *max_element(freq.begin(), freq.end());
                if(maxRem > (remaining + 1) / 2) { // check again if current character can be placed at position i and also the remaining max freq is less than (remaining string length + 1) / 2;
                    freq[ch - 'A'] += 1;
                    continue;
                }
                prev = ch;

...
void solveTowersOfHanoi(int n, int src, int dest, int aux) {
    if(n == 0) return;
    solveTowersOfHanoi(n - 1, src, aux, dest);
    vec.pb({src, dest});
    solveTowersOfHanoi(n - 1, aux, dest, src);
}
